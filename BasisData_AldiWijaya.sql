...
drop database basisdata_aldiwijaya;
...
DELIMITER //
create procedure updateDataProduk(updateid varchar(10), 
updatenama varchar(100), 
updatesatuan varchar(50), 
updateharga int(11), 
updatestok int(11)
)
BEGIN
    IF updatenama IS NOT NULL THEN
        UPDATE Produk SET namaproduk = updatenama WHERE idproduk = updateid;
    END IF;

    IF updatesatuan IS NOT NULL THEN
        UPDATE Produk SET satuan = updatesatuan WHERE idproduk = updateid;
    END IF;

    IF updateharga IS NOT NULL THEN
        UPDATE Produk SET harga = updateharga WHERE idproduk = updateid;
    END IF;

    IF updatestok IS NOT NULL THEN
        UPDATE Produk SET stok = updatestok WHERE idproduk = updateid;
    END IF;
END //
...
select * from pelanggan;
DELIMITER //
CREATE PROCEDURE UpdatePelanggan(updateid VARCHAR(10),
    updatenama VARCHAR(100),
    updatealamat VARCHAR(255),
    updatenotelp VARCHAR(15)
)
BEGIN
    IF updatenama IS NOT NULL THEN
        UPDATE Pelanggan SET nama = updatenama WHERE idpelanggan = updateid;
    END IF;

    IF updatealamat IS NOT NULL THEN
        UPDATE Pelanggan SET alamat = updatealamat WHERE idpelanggan = updateid;
    END IF;

    IF updatenotelp IS NOT NULL THEN
        UPDATE Pelanggan SET notelp = updatenotelp WHERE idpelanggan = updateid;
    END IF;
END //

DELIMITER ;
call updatePelanggan('PL-BD-002', 'Aldi wijaya', 'Palembang','082212345123');
-- CALL UpdateProduk('BP-BR-001', NULL, 'kg', 75000, 100); 
-- jadi jika data null maka tidak melakukan update data / nama akan di skip dari perubahan data
drop procedure updatePelanggan;

-- 11. Buatlah prosedur untuk Update data Transaksi (Data yang diupdate dapat apa saja selain dari primary key/ Foreign Key)
select * from transaksi;
DELIMITER //
CREATE PROCEDURE UpdateTransaksi(updateid VARCHAR(10),
    updatetanggal DATE
)
BEGIN
    IF updatetanggal IS NOT NULL THEN
        UPDATE Transaksi SET tanggal = updatetanggal WHERE idtransaksi = updateid;
    END IF;
END //
DELIMITER ;
CALL UpdateTransaksi('TR-01', '2026-04-10');
...
select * from detailTransaksi;
DELIMITER //
CREATE PROCEDURE UpdateDetailTransaksi(p_idtransaksi VARCHAR(10),
    p_idproduk VARCHAR(10),
    updatejumlah INT
)
BEGIN
    IF updatejumlah IS NOT NULL THEN
        UPDATE DetailTransaksi 
        SET jumlah = updatejumlah 
        WHERE idtransaksi = p_idtransaksi AND idproduk = p_idproduk;
    END IF;
END //
DELIMITER ;
CALL UpdateDetailTransaksi('TR-01', 'BP-BR-001', 9);
