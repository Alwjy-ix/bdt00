...
-- NamaLengkap mengandung huruf M ATAU N
SELECT * FROM Tamu
WHERE NamaLengkap REGEXP '[M-N]';

-- Huruf pertama harus A, B, atau C
SELECT * FROM Tamu
...
-- Tampilkan Namalayanantambahan dan Nama pegawai yang bertanggung jawab atas suatu layanan
...
-- Tampilkan nama tamu dan id tamu yang belum pernah memesan layanan tambahan
select Tamu.NamaLengkap, Tamu.IdTamu
from Tamu
left join PemesananLayanan
on Tamu.IDTamu = PemesananLayanan.IDTamu
where PemesananLayanan.IDTamu is null;
...
-- Tampilkan nama tamu dalam huruf kapital semua beserta panjang nama masing-masing
select upper(NamaLengkap) as NamaBesar,
length(NamaLengkap) as PanjangNama
from Tamu;

-- Tampilkan biaya layananTambahan lalu akar kuadratnya
select NamaLayanan,
Biaya,
sqrt(Biaya) as Akar
from LayananTambahan;
